...
const Background: FC<BackgroundProps> = ({ status = "inactive" }) => {
  return (
    <svg
      ...
      className={
        status
          ? `step__background step__background--${status}`
          : "step__background"
      }
    >
      <path d="M41 16H0" className="step__background__line" />
      <path d="M320 16H279" className="step__background__line" />
      <path
        ...
        fill="white"
        className="step__background__shape"
      />
    </svg>
  );
};
